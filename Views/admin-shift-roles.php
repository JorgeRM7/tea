                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12 order-0 order-md-1">
                                <div class="card">
                                    <div class="card-header d-flex justify-content-between align-items-center">
                                        <h5 class="mb-0">Roles de horario</h5>
                                    ...
                                    <form name="formulario" id="formulario" method="POST">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input type="hidden" id="shift_role_id" name="shift_role_id" class="form-control"/>
                                            </div>       
                                        </div>
                                    </form>
                                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        const menuItem = document.querySelector('a[href="admin-shift-roles.php"]').parentElement;
    ...
    const store = () => {
        ...
        $.ajax({
            url: "../Controllers/adminShiftRolesController.php?op=store",
            type: "POST",
            headers: {
    ...
    const index = () => {
        ...
        tabla = $('#tbllistado').dataTable({
            ...
            // "dom": 'Bfrtip',
            "ajax": {
                url: '../Controllers/adminShiftRolesController.php?op=index',
                type: "get",
                headers: {
                    "Authorization": "Bearer " + token
                },
    ...
    const show = ( shift_role_id ) => {
        $('#modal_create').modal('show');
        $.ajax({
            url: "../Controllers/adminShiftRolesController.php?op=show",
            type: "POST",
            headers: {
                "Authorization": "Bearer " + token
            },
            dataType: "json",
            data: { shift_role_id: shift_role_id },
            success: function (response) {
                let data = response;
                $("#shift_role_id").val(data?.id);
                $("#name").val(data?.name);                
            },
    ...
    const deleteItem = ( shift_role_id ) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "../Controllers/adminShiftRolesController.php?op=deleteItem",
                    type: "POST",
                    headers: {
                        "Authorization": "Bearer " + token
                    },
