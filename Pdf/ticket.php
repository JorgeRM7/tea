...
foreach ($tickets_ids as $index => $ticket_id) {
    ...
    <div class='ticket'>
        ...
        <table>
            <tr>
                ...
                <td rowspan='6' class='qr'><img src='{$url}' width='65'></td>
            </tr>
            <tr><td class='label'>Salida:</td><td class='value'>{$item['leaving_time']}</td></tr>
            <tr><td class='label'>Origen:</td><td class='value'>{$item['origin']}</td></tr>
            <tr><td class='label'>Destino:</td><td class='value'>{$item['destination']}</td></tr>
            <tr><td class='label'>Precio:</td><td class='value'>$ {$total}</td></tr>
            <tr><td class='label'>Unidad:</td><td class='value'>{$item['vehicle_id']}</td></tr>
            <tr><td class='label'>Operador:</td><td class='value'>{$item['employee']}</td></tr>
            <tr><td class='label'>Hora venta:</td><td class='value'>{$item['created_at']}</td></tr>
        </table>

        <hr>
        <p style='font-size:8px;text-align:center;'>¡Es un placer servirle!</p>
    </div><br>
    ";
    ...
    $mpdf->WriteHTML($html_cliente);
    
    $mpdf->WriteHTML($html_operador);

    if ($index < count($tickets_ids) - 1) {
        $mpdf->AddPage();
    }
}
